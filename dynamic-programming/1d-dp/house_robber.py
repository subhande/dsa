...
class Solution:
    def houseRobber(self, house_values):
        ...
        max_sum_previous_house = max(
            house_values[0], house_values[1]
        )  # Maximum money robbed up to the previous house

        # When the first house is not picked
        max_sum_two_houses_ago_no_first = (
            0  # Maximum money robbed up to two houses ago, excluding the first house
        )
        max_sum_previous_house_no_first = house_values[
            1
        ]  # Maximum money robbed up to the previous house, excluding the first house
        ...
        for current_index in range(2, num_houses):
            if current_index == num_houses - 1:
                ...
                current_max_sum = max(
                    house_values[current_index] + max_sum_two_houses_ago, max_sum_previous_house
                )

            # Calculate the maximum sum when the first house is not picked
            current_max_sum_no_first = max(
                house_values[current_index] + max_sum_two_houses_ago_no_first,
                max_sum_previous_house_no_first,
            )
