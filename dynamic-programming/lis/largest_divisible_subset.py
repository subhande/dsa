...
class Solution:
    def largestDivisibleSubsetRecursiveMemo(
        self, nums: List[int], prev: int, curr: int, memo: List
    ) -> list:
        if curr == len(nums):
            return []
        ...
        if prev == -1 or nums[curr] % nums[prev] == 0 or nums[prev] % nums[curr] == 0:
            taken = [nums[curr]] + self.largestDivisibleSubsetRecursiveMemo(
                nums, curr, curr + 1, memo
            )
