...
class Solution:
    ...
    def longestCommonSubstrRecursiveHelper(self, str1, str2, ind1, ind2, currentSubstrLen):
        ...
        if str1[ind1] == str2[ind2]:
            return self.longestCommonSubstrRecursiveHelper(
                str1, str2, ind1 - 1, ind2 - 1, currentSubstrLen + 1
            )

        return max(
            currentSubstrLen,
            max(
                self.longestCommonSubstrRecursiveHelper(str1, str2, ind1 - 1, ind2, 0),
                self.longestCommonSubstrRecursiveHelper(str1, str2, ind1, ind2 - 1, 0),
            ),
        )
    ...
    def longestCommonSubstrMemoizationHelper(self, str1, str2, i, j, memo):
        ...
        if str1[i] == str2[j]:
            memo[i][j] = 1 + self.longestCommonSubstrMemoizationHelper(
                str1, str2, i - 1, j - 1, memo
            )
        else:
    ...
    def longestCommonSubstrMemoization(self, str1, str2):
        ...
        for i in range(n):
            for j in range(m):
                length_ending_here = self.longestCommonSubstrMemoizationHelper(
                    str1, str2, i, j, memo
                )
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        {"str1": "abcdef", "str2": "ghijkl", "output": 0},
    ]

    for i, test_case in enumerate(test_cases):
        print("==================================")
        print(f"Test case {i + 1}")
        outputRecusive = longestCommonSubstr.longestCommonSubstrRecursive(
            test_case["str1"], test_case["str2"]
        )
        print(f"Output (Recursive): {outputRecusive}")
        outputMemoization = longestCommonSubstr.longestCommonSubstrMemoization(
            test_case["str1"], test_case["str2"]
        )
        print(f"Output (Memoization): {outputMemoization}")
        outputTabulation = longestCommonSubstr.longestCommonSubstrTabulation(
            test_case["str1"], test_case["str2"]
        )
        print(f"Output (Tabulation): {outputTabulation}")
        outputTabulationSpaceOptimized = (
            longestCommonSubstr.longestCommonSubstrTabulationSpaceOptimized(
                test_case["str1"], test_case["str2"]
            )
        )
