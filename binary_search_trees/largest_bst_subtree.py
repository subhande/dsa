...
class Solution1:
    ...
    def largestBSTSubtreeHelper(self, node: TreeNode | None) -> tuple:
        ...
        isLeftSubTreeValidBST, leftSubTreeSize, leftMin, leftMax = self.largestBSTSubtreeHelper(
            node.left
        )
        # Recursive Case: Check if the left and right subtrees are valid BSTs
        isRightSubTreeValidBST, rightSubTreeSize, rightMin, rightMax = self.largestBSTSubtreeHelper(
            node.right
        )
        ...
        if valid and isLeftSubTreeValidBST and isRightSubTreeValidBST:
            return (
                True,
                1 + leftSubTreeSize + rightSubTreeSize,
