...
class Solution:
    def combinationSum3Recursive(
        self, k: int, n: int, index: int, nums: list, currCombination: list, validCombinations: list
    ):
        if n == 0:
            if k == 0:
        ...
        self.combinationSum3Recursive(
            k - 1,
            n - nums[index],
            index - 1,
            nums,
            [nums[index]] + currCombination,
            validCombinations,
        )
