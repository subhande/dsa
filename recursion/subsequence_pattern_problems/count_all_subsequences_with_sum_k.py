...
class Solution:
    def countSubsequenceWithTargetSumRecursive(self, nums, k, currSum, index):
        ...
        return self.countSubsequenceWithTargetSumRecursive(
            nums, k, currSum - nums[index], index - 1
        ) + self.countSubsequenceWithTargetSumRecursive(nums, k, currSum, index - 1)
...
class SolutionMemo:
    def countSubsequenceWithTargetSumRecursive(self, nums, k, currSum, index, memo):
        ...
        memo[(currSum, index)] = self.countSubsequenceWithTargetSumRecursive(
            nums, k, currSum - nums[index], index - 1, memo
        ) + self.countSubsequenceWithTargetSumRecursive(nums, k, currSum, index - 1, memo)
        return memo[(currSum, index)]
