...
class Solution:
    def checkSubsequenceSumRecursive(self, nums, k, currSum, index):
        ...
        return self.checkSubsequenceSumRecursive(
            nums, k, currSum - nums[index], index - 1
        ) or self.checkSubsequenceSumRecursive(nums, k, currSum, index - 1)
...
class SolutionMemo:
    def checkSubsequenceSumRecursive(self, nums, k, currSum, index, memo):
        ...
        memo[(currSum, index)] = self.checkSubsequenceSumRecursive(
            nums, k, currSum - nums[index], index - 1, memo
        ) or self.checkSubsequenceSumRecursive(nums, k, currSum, index - 1, memo)
        return memo[(currSum, index)]
